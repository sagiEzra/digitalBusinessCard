    ...
    useEffect(() => {
        const handleScroll = () => {
            benefitRef.current.forEach((el, index) => {
                if (el && !hasSeen[index] && el.getBoundingClientRect().top < window.innerHeight) {
                    el.classList.add(styles.visible);
                    setHasSeen((prev) => ({ ...prev, [index]: true }));
                }
            });
            stepRef.current.forEach((el, index) => {
                if (el && !hasSeen[`step-${index}`] && el.getBoundingClientRect().top < window.innerHeight) {
                    el.classList.add(styles.visible);
                    setHasSeen((prev) => ({ ...prev, [`step-${index}`]: true }));
                }
            });
        };
    ...
    return (
        <div className={styles.container}>
            ...
            <div className={styles.benefitList}>
                {[
                { icon: <FaClock />, text: "יצירה מהירה תוך דקות" },
